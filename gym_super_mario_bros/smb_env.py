...
class SuperMarioBrosEnv(NESEnv):
    ...
    def __init__(self, rom_mode='vanilla', lost_levels=False, target=None):
        ...
        #TODO: add basic attributes to modify the reward function
        self._coins_last = 0
        self._level_last = 0
        self._score_last = 0
        # reset the emulator
    ...
    #TODO: set a new reward rule
    @property
    def _coin_reward(self):
        _reward = (self._coins - self._coins_last) * 100
    ...
    @property
    def _level_reward(self):
        _reward = (self._level > self._level_last) * 2000
        self._level_last = self._level
        if _reward < 0:
            return 0
        return _reward
    ...
    def _get_reward(self):
        ...
        #TODO: use time and death penalty only
        # print(self._x_reward, self._level_reward, self._coin_reward, self._time_penalty, self._death_penalty)
        coin = self._coin_reward
        level = self._level_reward
        score = self._score_reward
        death = self._death_penalty
        
        return death + coin + level + score
